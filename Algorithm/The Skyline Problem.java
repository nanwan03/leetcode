public class Solution {
    ...
    public List<int[]> getSkyline(int[][] buildings) {
        ...
        Queue<Node> heap = new PriorityQueue<Node>(2 * buildings.length, new NodeComparator());
        for (Rectangle r : rst) {
            heap.offer(new Node(r.left, r.height));
            heap.offer(new Node(r.right, 0));
        }
        Set<Integer> xAxis = new HashSet<Integer>();
        while (!heap.isEmpty()) {
            Node n = heap.poll();
            if (xAxis.add(n.x)) {
                output.add(new int[]{n.x, n.y});
            }
        }
        return output;
    }
